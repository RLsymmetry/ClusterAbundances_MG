...
class Cosmosground(object):
    ...
    def _p(self, z, w0 = -1, wa = 0, npoints = 513):
        ...
        parms.set_cosmology(H0 = self._H0, omch2 = self._omch2, ombh2 = self._ombh2, tau = 0.06, mnu = 0.06) 
    ...
    def sigma8_model_ratio(self, model, z, k = None):
        ...
        #Getting the sigma8_lcdm-z relation at redshift zero
        ...
        '''
        #This is the old normalizing at redshift 0 scheme.
        #print('In sigma8_model_ratio, the Om used is:')
        #print(self._Om)
        #Getting the sigma8_lcdm at redshift zero
        if z[0] == 0:
            z_aug = z
        else:
            z_aug = np.append(np.array([0]), z)
        s8lcdm = self._sigma8_dyn(z_aug)[1]
        D0 = LCDM().solvegrowth(np.linspace(0, 3, 30))[0]#D_{ΛCDM}(k,z=0), solved from the LCDM subclass; 
                                                         #used 30 redshift bins (actually can use less)
        ...
        if model.kdependence == 0:           
            ...
            if z[0] == 0:
                ...
                fratio = np.delete(ratio, 0)
                fs8 = np.delete(s8d, 0)
            return fratio, fs8
        elif model.kdependence == 1:
            pkarray = self._Pk(z_aug, k, model.solvegrowth(z_aug, k))
            s8model = self._sigma8_dyn(z_aug, pkarray = pkarray, npoints = len(k))[1]
            sratio0 = s8model[0]/s8lcdm[0]
            norms8model = s8model/sratio0
            ratio = norms8model/s8lcdm
            if z[0] == 0:
                fratio = ratio
                fs8 = norms8model
            else:
                fratio = np.delete(ratio, 0)
                fs8 = np.delete(norms8model, 0)
            return fratio, fs8
        '''
        
        
        '''
        #This is unnormalized scheme.
        if model.kdependence == 0:
            #I assume, since for f(R) models we have 
            #P_{MG}(k,z) = P_{ΛCDM}(k,z=0)*(D_{MG}(k,z)/D_{ΛCDM}(z=0))^2, 
            #then for DGP models where there is no k dependence, 
            #σ_{MG}(z) = σ_{ΛCDM}(z=0)*(D_{MG}(z)/D_{ΛCDM}(z=0)).
            #Extremely not sure if this is correct, but I'll try.
            D = model.solvegrowth(z_aug)
            s8d = s8lcdm[0] * D/D0
            ratio = s8d/s8lcdm
            if z[0] == 0:
            ...
            pkarray = self._Pk(z_aug, k, model.solvegrowth(z_aug, k), model)
            s8model = self._sigma8_dyn(z_aug, pkarray = pkarray, npoints = len(k))[1]
            ratio = s8model/s8lcdm
            if z[0] == 0:
                fratio = ratio
                fs8 = s8model
            else:
                fratio = np.delete(ratio, 0)
                fs8 = np.delete(s8model, 0)
            return fratio, fs8
        '''
...
class HuSawicki(Cosmosground):
    ...
    def setpars(self, pars):
        ...
        pars: a numpy array of parameters. (Looks like lists and tuples are also acceptable...)
        """
    ...
    def _m(self, a):
        ...
        return (1/self._c) * np.sqrt(1/((self._n + 1) * self._fr0)) * \
               np.sqrt((self._Om + 4 * (1 - self._Om)) ** (- self._n - 1)) * np.sqrt((self._Om/(a ** 3) \
               + 4 * (1 - self._Om)) ** (2 + self._n))
    ...
    def _frtestderiv(self, fr0_exp, n, z, k, save):
        ...
        """
        self.set_n(n)
        print('Testing partial derivative over f_R0')
        ...
        if fr0_exp == 'NegInfty':
            ...
            sfder1 = (sf12-sf11)/1e-9

            #Numerical derivative with half stepsize 10^(-10)
            self.set_fr0(0)
    ...
    def _ntestderiv(self, fr0_exp, n, z, k, save):
        ...
        self.set_n(n * 0.9)
        sn11 = self.sigma8_ratio(self, z, k)
        self.set_n(n * 1.1)
        sn12 = self.sigma8_ratio(self, z, k)
        snder1 = (sn12-sn11)/(n * 0.2)

        #Numerical derivative with half stepsize 5%
        self.set_n(n * 0.95)
        sn21 = self.sigma8_ratio(self, z, k)
        self.set_n(n * 1.05)
