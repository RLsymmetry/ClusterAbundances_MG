...
#import os
#os.chdir('/Users/RayneLiu/Desktop/Research/Cosmology/Growth \
...
class Cosmosground(object):
    ...
    def __init__(self):
        ...
        self._amax = 1
        self._c = 2997
        self._H0 = 67.0 #H0 = 67.0, i.e. h = 0.67. However, this module itself works on normalized H0 = 1, and 67.0 is mainly for CAMB.) #67.77#
        self._omch2 = 0.1194 #0.11889
        self._ombh2 = 0.022
    ...
    def _p(self, z, w0 = -1, wa = 0, npoints = 513):
        ...
        parms.set_cosmology(H0 = self._H0, omch2 = self._omch2, ombh2 = self._ombh2, tau = 0.06, mnu = 0.064) #mnu = 0.06
    ...
    def sigma8_check(self, z):
        ...
        print(sigma8_camb)
        print('The ratio of the former against the latter is:')
        print(check_ratio)
    ...
    def sigma8_model_ratio(self, model, z, k = None):
        ...
        (* Updated 11/12/2020: to build more consistency, in the function sigma8_ratio below we change the σ8_{ΛCDM} to that came from the LCDM model class. *)
        ...
        if model.kdependence == 0:
            ...
            #D/D[0] is normalizing the growth factor, since the growth factor in any model is 1 at redshift 0.
            #(Actually this probably is not necessary...we normalized σ8 later anyways. I tested it's not. But I have OCD..)
            D_norm = D/D[0]
            #However, here when we normalize the s8 ratio wrt z = 10,
            #s8_MG(z=10) = s8_LCDM(z=10). Hence s8_MG(z=0) = s8_MG(z=10)/D_norm(z=10) = s8_LCDM(z=10)/D_norm(z=10).
            #We then multiply s8_MG(z = 0) by D_norm, which is 1 at z = 0.
            s8d = (s8lcdm[-1]/D_norm[-1]) * D_norm
            #And then take the ratio over s8lcdm. This ratio should already be 1 at redshift 10,
            #hence the s8 should already be a normalized one, which I can directly use.
            ratio = s8d/s8lcdm
    ...
    def sigma8_ratio(self, model, z, k = None):
        """
        Returns: the ratio from sigma8_model_ratio alone, taking the ratio over the sigma8 values obtained from the LCDM class (Instead of CAMB).
        Normalization checked from the plot.
        """
        s8lcdm_growth = self.sigma8_model_ratio(LCDM(), z, k)[0]#The fs8 from the growth LCDM model
        return self.sigma8_model_ratio(model, z, k)[0]/s8lcdm_growth
...
class HuSawicki(Cosmosground):
    ...
    def _frtestderiv(self, fr0_exp, n, z, k, save):
        ...
        if fr0_exp == 0:
            ...
            #$f'(x) = \frac{18f(x + h) - 9f(x + 2h) + 2f(x + 3h) - 11f(x)}{6h}$, which leaves only the O(h^4) terms
            #(i.e. 3rd-order accuracy).
    ...
    def _frtestderiv2(self, fr0_exp, n, z, k, save):
        ...
        """
        self.set_n(n)
        print('Testing partial derivative (2nd version) over f_R0')
...
class DGP(Cosmosground):
    ...
    It used to include the sDGP and nDGP model because the difference of sDGP from nDGP is the β term which is off by a sign, and also a different Hubble parameter (expansion history). This was done just for testing against the Linder paper. But after the test, the sDGP case is removed.
    Currently solves the growth differential equation and gives σ8 values, but can have more features.
    """
    ...
    def _rctestderiv(self, n, z, save):
        """
        Tests the convergence of the numerical derivative partial n (n = H0 * rc here) in the DGP model,
        but is called by rctestderiv in order to distinguish between the _ntestderiv in f(R)
        (Also trying to avoid confusion in other modules that already called this function). Inherited parameters.
        """
        print('Testing partial derivative over n')
        steps = np.array([0.1, 0.05])
        srder = []
